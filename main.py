...
DEFAULT_SECRET_FILE = Path(r"C:\새 홈페이지\AI_Secrets\handan-ai-agent\gemini.toml")
GEMINI_MODEL = "gemini-3.6-flash"


def load_gemini_api_key() -> str:
    configured_path = os.getenv("HANDAN_GEMINI_SECRET_FILE")
    secret_file = Path(configured_path) if configured_path else DEFAULT_SECRET_FILE

    if not secret_file.exists():
        raise FileNotFoundError(
            f"Gemini Secret 파일을 찾을 수 없습니다: {secret_file}"
        )
    ...
    if not isinstance(api_key, str) or not api_key.strip():
        raise KeyError("gemini.toml에 GEMINI_API_KEY가 없거나 비어 있습니다.")
...
if user_prompt:
    ...
    try:
        ...
        st.session_state.messages.append({
            "role": "assistant",
            "content": response.text or "답변을 생성하지 못했습니다.",
        })
        st.rerun()

    except FileNotFoundError as e:
        st.error(f"API 키 파일 오류: {e}")
